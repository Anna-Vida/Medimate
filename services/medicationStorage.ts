...
function getMedicationStorageKey(): string {
  return getUserScopedKey(MEDICATION_STORAGE_KEY);
}

function getReminderStorageKey(): string {
  return getUserScopedKey(REMINDER_STORAGE_KEY);
}
...
export async function getAllMedications(): Promise<MedicationRecord[]> {
  try {
    const data = await AsyncStorage.getItem(getMedicationStorageKey());
    if (!data) return [];
...
export async function markMedicationTaken(id: string): Promise<void> {
  try {
    ...
    if (index !== -1) {
      ...
      if (
        medications[index].inventoryCount !== undefined &&
        medications[index].inventoryCount! > 0
      ) {
...
export async function updateMedicationInventory(
  ...
  try {
    ...
    const index = medications.findIndex(
      (med) =>
        med.analysis.medicineName === medicineName && med.status === "active",
    );
...
export async function getTodaySchedule(): Promise<DailySchedule[]> {
  ...
  for (const med of activeMeds) {
    if (med.analysis.recommendedTime) {
      const taken = med.lastTaken && med.lastTaken.getTime() >= today.getTime();
...
export async function clearAllMedications(): Promise<void> {
  try {
    await AsyncStorage.removeItem(getMedicationStorageKey());
  } catch (error) {
...
export async function saveReminder(
  reminder: Omit<ReminderSchedule, "id" | "createdAt" | "color">,
  ...
  try {
    ...
    await AsyncStorage.setItem(getReminderStorageKey(), JSON.stringify(all));
    return newReminder;
  } catch (error) {
...
export async function getAllReminders(): Promise<ReminderSchedule[]> {
  try {
    const data = await AsyncStorage.getItem(getReminderStorageKey());
    if (!data) return [];
