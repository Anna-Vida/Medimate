...
export const saveScan = async (
  ...
  try {
    ...
    await AsyncStorage.setItem(getScansKey(), JSON.stringify(updatedScans));

    // ── Auto-create reminders for each scanned medicine ──
    try {
      const existingReminders = await getAllReminders();
      const existingNames = existingReminders.map((r) =>
        r.medicineName.toLowerCase(),
      );
      ...
      for (const med of analysis) {
        ...
        const dose =
          med.dosage && med.dosage !== "Not visible" ? med.dosage : "";
        ...
        if (
          instr.includes("three times") ||
          instr.includes("3 times") ||
          instr.includes("tid") ||
          instr.includes("3x")
        ) {
          frequency = "3x Daily";
        } else if (
          instr.includes("twice") ||
          instr.includes("two times") ||
          instr.includes("2 times") ||
          instr.includes("bid") ||
          instr.includes("2x")
        ) {
          frequency = "2x Daily";
...
export const getRecentScans = async (): Promise<SavedScan[]> => {
  try {
    const json = await AsyncStorage.getItem(getScansKey());
    if (!json) return [];
...
export const clearScans = async (): Promise<void> => {
  try {
    await AsyncStorage.removeItem(getScansKey());
  } catch (error) {
