...
export default function MedicineDetailsScreen() {
  ...
  const scan: SavedScan | null = params.scanData
    ? JSON.parse(params.scanData as string)
    : null;
  ...
  useEffect(() => {
    const loadInventory = async () => {
      try {
        ...
        const data: Record<string, { count: number; daily: number }> = {};
        for (const m of activeMeds) {
          if (m.inventoryCount !== undefined && m.inventoryCount !== null) {
            data[m.analysis.medicineName.toLowerCase()] = {
              count: m.inventoryCount,
              daily: m.dailyDoseCount || 1,
            };
          }
        }
  ...
  const handleSpeak = async (text: string, fieldId: string, index?: number) => {
    try {
      ...
      if (index !== undefined && translatedMeds[index]) {
        if (fieldId.includes("purpose"))
          toSpeak = translatedMeds[index].purpose;
        if (fieldId.includes("warning"))
          toSpeak = translatedMeds[index].warnings;
  ...
  const handleSetReminder = async (medicine: MedicineAnalysis) => {
    ...
    Alert.alert(
      ...
      [
        ...
        {
          text: "Set for 8:00 AM",
          onPress: () => scheduleSimpleReminder(medicine.medicineName, 8, 0),
        },
        {
    ...
    <View style={styles.container}>
      ...
      >
          ...
          >
            <Text style={styles.heroTimestamp}>
              ...
              {scan.timestamp
                ...
                : new Date(Number(scan.id)).toLocaleDateString()}
            </Text>
          </LinearGradient>
        </View>
        ...
        {meds.map((med, index) => {
          ...
          const displaySimpleInstructions =
            tx?.simpleInstructions || med.simpleInstructions;
            ...
            <View key={index} style={styles.medSection}>
              ...
              <View style={styles.titleCard}>
                ...
                <View style={{ flex: 1 }}>
                  ...
                  {med.dosage && (
                    <Text style={styles.dosageLabel}>{med.dosage}</Text>
                  )}
                </View>
                <TouchableOpacity
                  onPress={() =>
              ...
              <View style={styles.infoCard}>
                <View style={styles.cardHeader}>
                  ...
                  <TouchableOpacity
                    onPress={() =>
                      handleSpeak(displayPurpose, `purpose-${index}`, index)
                    }
                    style={styles.ttsBtn}
                  >
              ...
              {displayWarnings && (
                <View style={[styles.infoCard, styles.warningCard]}>
                  <View style={styles.cardHeader}>
                    ...
                    <TouchableOpacity
                      onPress={() =>
                        handleSpeak(displayWarnings, `warning-${index}`, index)
                      }
                      style={styles.ttsBtn}
                    >
              ...
              <View style={styles.infoCard}>
                ...
                <Text style={styles.usageText}>
                  {med.recommendedTime || "As specified by doctor"}
                </Text>
              </View>
          ...
          <View style={styles.pickerCard}>
            ...
            {editingMedIndex !== null && meds[editingMedIndex] && (
              ...
              >
                ...
                <TouchableOpacity
                  onPress={() => setShowInventorySheet(false)}
