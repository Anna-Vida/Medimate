...
export default function ScanHistoryScreen() {
    ...
    const handleMedicinePress = (medicineName: string, dosage?: string) => {
        setSelectedMeds(prev => {
            const exists = prev.some(m => m.name.toLowerCase() === medicineName.toLowerCase());
            if (exists) {
                return prev.filter(m => m.name.toLowerCase() !== medicineName.toLowerCase());
            }
            if (prev.length >= 2) {
                Alert.alert('Limit Reached', 'Select up to 2 medicines to check interactions');
                return prev;
            }
    ...
    const checkInteraction = async () => {
        if (selectedMeds.length < 2) {
            Alert.alert('Select Medicines', 'Please select 2 different medicines to check interactions');
            return;
        }

        try {
            setCheckingInteraction(true);
            const result = await checkDrugInteraction(selectedMeds[0].name, selectedMeds[1].name);
            ...
            Alert.alert('Error', 'Failed to check drug interactions. Please verify your medicines and try again.');
        } finally {
    ...
    const getStatusInfo = (warnings?: string) => {
        if (!warnings || warnings === 'None') return { label: 'Safe', color: Colors.success, bg: Colors.primaryBg };
        const w = warnings.toLowerCase();
        if (w.includes('severe') || w.includes('stop')) return { label: 'Warning', color: Colors.error, bg: Colors.errorBg };
        if (w.includes('caution') || w.includes('avoid')) return { label: 'Caution', color: Colors.warning, bg: Colors.warningBg };
                ...
                <View style={styles.headerTop}>
                    ...
                    {scans.length > 0 && (
                        <View style={{ flexDirection: 'row', gap: 8 }}>
                            ...
                            <TouchableOpacity onPress={handleClearAll} style={[styles.headerAction, { backgroundColor: Colors.surfaceHighlight, borderColor: Colors.border, borderWidth: 1 }]} activeOpacity={0.7}>
                    ...
                    <View style={styles.selectedBar}>
                        <View style={styles.selectedMeds}>
                            {selectedMeds.map((med, idx) => (
                                ...
                                >
                                    <Text style={styles.selectedMedText} numberOfLines={1}>{med.name}</Text>
                                        ...
                                        >
                                            <View style={[styles.rxRow, !isLast && styles.rxRowBorder]}>
                                                ...
                                                <View style={styles.rxInfo}>
                                                    {meds.map((m, mi) => {
                                                        const isSelected = selectedMeds.some(sm => sm.name.toLowerCase() === m.medicineName.toLowerCase());
                                                        return (
                                                            <TouchableOpacity
                                                                key={mi}
            ...
            >
                <SafeAreaView style={styles.modalContainer}>
                    <View style={styles.modalHeader}>
                        <Text style={styles.modalTitle}>Drug Interaction Check</Text>
                        <TouchableOpacity
                            onPress={() => {
                                setShowInteractionModal(false);
                                clearSelection();
                    ...
                    {checkingInteraction ? (
                        <View style={styles.loadingContainer}>
                            <ActivityIndicator size="large" color={Colors.primary} />
                            <Text style={styles.loadingText}>Checking drug interaction...</Text>
                        </View>
                    ) : interactionResult ? (
                        <ScrollView style={styles.modalContent} showsVerticalScrollIndicator={false}>
                            ...
                            {/* Interaction Status */}
                            <View style={[styles.statusSection, { backgroundColor: interactionResult.isSafe ? Colors.primaryBg : Colors.errorBg }]}>
                                <View style={{ flexDirection: 'row', alignItems: 'center', gap: 12 }}>
                                    <View style={[styles.statusIcon, { backgroundColor: interactionResult.isSafe ? Colors.success : Colors.error }]}>
                                        <Ionicons
                                    ...
                                    <View style={{ flex: 1 }}>
                                        <Text style={styles.statusTitle}>
                                            {interactionResult.isSafe ? 'Safe to Combine' : 'Interaction Detected'}
                                        </Text>
...
const styles = StyleSheet.create({
    ...
    headerTitle: { fontSize: moderateScale(22), fontWeight: '700', color: Colors.textPrimary },
    headerSub: { fontSize: moderateScale(13), fontWeight: '500', color: Colors.textSecondary, marginTop: 2 },
    headerAction: {
    ...
    searchInput: {
        flex: 1, fontSize: moderateScale(14), color: Colors.textPrimary, fontWeight: '500',
    },
    ...
    // Date Group Label
    dateLabel: {
        fontSize: moderateScale(13), fontWeight: '700', color: Colors.textTertiary,
    ...
    rxCard: {
        ...
        borderWidth: 1, borderColor: Colors.border,
        shadowColor: Shadows.small?.shadowColor || '#000', shadowOffset: { width: 0, height: 2 },
    ...
    rxText: {
        fontSize: moderateScale(14), fontWeight: '800', color: Colors.primary,
        fontStyle: 'italic',
    },

    // Medicine info
    rxInfo: { flex: 1 },
    rxName: { fontSize: moderateScale(15), fontWeight: '700', color: Colors.textPrimary },
    rxDosage: { fontSize: moderateScale(12), fontWeight: '500', color: Colors.textSecondary, marginTop: 1 },
    rxMore: { fontSize: moderateScale(12), fontWeight: '600', color: Colors.primary, marginTop: 4 },
    ...
    rxTime: { fontSize: moderateScale(11), fontWeight: '500', color: Colors.textTertiary },
    ...
    emptySub: { fontSize: moderateScale(14), color: Colors.textTertiary, fontWeight: '500', textAlign: 'center', paddingHorizontal: 40 },
    emptyBtn: {
        flexDirection: 'row', alignItems: 'center', gap: 8,
        backgroundColor: Colors.primary, paddingHorizontal: scale(24),
